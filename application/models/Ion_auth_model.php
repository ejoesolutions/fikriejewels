...
class Ion_auth_model extends CI_Model
{
	...
	*/
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ',username, email, id, password, active, last_login, user_group, verify_acc, removable, full_name')
		->join('ci_users_profile', 'ci_users_profile.user_id = '.$this->tables['users'].'.id')
		->where($this->identity_column, $identity)
		->limit(1)
		->order_by('id', 'desc')
		->get($this->tables['users']);
		...
		if ($query->num_rows() === 1)
		{
			...
			if ($password === TRUE)
			{
				if ($user->active == 0 || $user->verify_acc == 0 || $user->removable != null)
				{
					$this->trigger_events('post_login_unsuccessful');
	...
	public function login_cust($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ',username, email, id, password, active, last_login, user_group, verify_acc,removable')
		->where($this->identity_column, $identity)
		...
		if ($query->num_rows() === 1)
		{
			...
			if ($password === TRUE)
			{
				if ($user->active == 0 || $user->verify_acc == 0 || $user->removable != null || $user->user_group < 3)
				{
					$this->trigger_events('post_login_unsuccessful');
					$this->set_error('login_unsuccessful_not_active');

...
if ($date_min != null) {
  ...
  $pdf->Text(166.6, 37, 'Bulan '. $date_month .'-'. $date_year);
}
...
<thead>
  <tr style="background-color:#F4F6F7;color:black;" align="center">
    <td width="98"><b>No.Siri</b></td>
    <td width="110"><b>Produk</b></td>
    <td width="60"><b>Berat (g)</b></td>
    <td width="65"><b>Jumlah (RM)</b></td>
    <td width="72"><b>Upah (RM)</b></td>
    <td width="72"><b>Kod Jualan</b></td>
    <td width="80"><b>Jumlah (RM)</b></td>
    <td width="70"><b>Tarikh</b></td>
  </tr>
</thead>
';
$count_all = 0;
if ($variants) {
  foreach($variants as $key) {
    ...
    $html .='</td>
    <td width="110">'.$key['product_name'].'</td>
    <td width="60">'.number_format($key['ordered_weight'], 2, '.', '').'</td>
    <td width="65">'.number_format($key['subtotal'], 2, '.', '').'</td>
    <td width="72">'.number_format($key['ordered_margin_pay'], 2, '.', '').'</td>';
    if (is_numeric($key['v_kod'])) {
      $html .= '<td width="72">'.number_format($key['v_kod'] / 3, 2,'.','').'</td>
      <td width="80">'.number_format($key['ordered_margin_pay'] - ($key['v_kod'] / 3),2,'.','').'</td>';
    }else {
      $html .= '<td width="72">0</td>
      <td width="80">'.number_format($key['ordered_margin_pay'] - 0,2,'.','').'</td>';
    }
    $html .= '
    <td width="70">'.date("d-m-Y", strtotime($key['updated'])).'<br>'.date("h:i a", strtotime($key['updated'])).'</td>
    </tr>
    ';
  }
}

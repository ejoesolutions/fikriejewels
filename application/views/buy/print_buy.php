...
foreach ($maklumat as $key) { 
  ...
  if ($key['n_tambahan']) {
    ...
    $pdf->Text(159, 33, 'PENYATA BELIAN');
...
$pdf->Text(145, 42, 'Tarikh Dicetak : '.date('d-m-Y'));
...
<thead>
  <tr style="background-color:#F4F6F7;color:black;" align="center">
    ...
    <td width="185"><b>Produk</b></td>
    <td width="80"><b>Harga (RM)</b></td>
    <td width="60"><b>Berat (g)</b></td>
    <td width="60"><b>Mutu</b></td>
    <td width="140"><b>Tarikh</b></td>
  </tr>
</thead>
';
if ($buy) {
    foreach($buy as $key) {
      $html .= '
      <tr align="center">
        <td width="100">'.$key['serial_number'].'</td>
        <td width="185">'.$key['product_name'].'</td>
        <td width="80">'.$key['harga'].'</td>
        <td width="60">'.$key['berat'].'</td>
        <td width="60">'.$key['mutu'].'</td>
        <td width="140">'. date('d-m-Y',strtotime($key['created_date'])) .' '.date('h:i a',strtotime($key['created_date'])).'</td>
      </tr>
      ';
    }
}
...
<tr>
  ...
  <td width="185"><h3>: '. $count_total_buy['total'] .'</h3></td>
...
$pdf->Output('Penyata Belian.pdf', 'I');

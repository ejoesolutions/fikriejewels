...
function MergeCSS($inherit,$tag,$attr) {
	...
	// mPDF 6
	$shortlang = '';
	//if (!isset($attr['LANG']))   asal
	if (is_array($attr) &&!isset($attr['LANG']))  //ubah
	{ $attr['LANG']=''; }
	else {
		// $attr['LANG'] = strtolower($attr['LANG']); asal
		// if (strlen($attr['LANG']) == 5) { asal
		// 	$shortlang = substr($attr['LANG'],0,2); asal
 		// } asal
		if(is_array($attr)){
...
/*-- END TABLES --*/
	...
	if ($inherit == 'BLOCK') {
		...
		$attrID     = isset($attr['ID']) ? $attr['ID'] : false; //ubah
		$attrLANG   = isset($attr['LANG']) ? $attr['LANG'] : false; //ubah


		//$this->_mergeFullCSS($this->cascadeCSS, $this->mpdf->blk[$this->mpdf->blklvl]['cascadeCSS'], $tag, $classes, $attr['ID'], $attr['LANG']); asal
		$this->_mergeFullCSS($this->cascadeCSS, $cascadeCSS, $tag, $classes, $attrID, $attrLANG); //ubah
		//===============================================
		// Cascading forward CSS
		//===============================================
		if (isset($this->mpdf->blk[$this->mpdf->blklvl-1])) {
			//asal $this->_mergeFullCSS($this->mpdf->blk[$this->mpdf->blklvl-1]['cascadeCSS'], $this->mpdf->blk[$this->mpdf->blklvl]['cascadeCSS'], $tag, $classes, $attr['ID'], $attr['LANG']);
			$this->_mergeFullCSS($this->mpdf->blk[$this->mpdf->blklvl - 1]['cascadeCSS'], $cascadeCSS, $tag, $classes, $attrID, $attrLANG);//ubah
		}
		//===============================================
